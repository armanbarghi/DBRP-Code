...
def buffer_optimality(env, action_seq, initial_scene, target_scene, verbose=0):
	...
	# Check for stack actions in simple refinement
	for a in action_seq:
		if a['type'] == 'stack':
	...
	for i in range(len(action_seq)):
		...
		if k in B:			# if object k was moved
			...
			for p in P:
				...
				if torch.equal(p.to(torch.long), action_seq[i]['p_place']):
					continue
			...
			# Update the best buffer using unified format
...
def buffer_optimality_stack(env, action_seq, initial_scene, target_scene, verbose=0):
	...
	for i in range(len(action_seq)):
		...
		if k in B:			# if object k was moved
			...
			# Occupied moving buffers in the action index b to i-1
			empty_objs = []
